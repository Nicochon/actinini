...
-- ------------------------------------------------------------
-- TRIGGER 5 bis — un seul créneau, retenu d'office
-- Une sortie à date unique n'a rien à faire trancher : le
-- créneau est la date. L'invariant est tenu ici plutôt qu'à
-- l'affichage, pour que la liste, le calendrier, le badge de
-- statut et la page de détail lisent la même vérité.
-- ------------------------------------------------------------
create function confirm_lone_date_option()
returns trigger
language plpgsql security definer set search_path = public, pg_temp
as $$
declare
  ...
  if total = 1 then
    ...
    update activities
       set confirmed_date_option_id = lone,
           -- Une activité passée ou annulée garde son statut.
